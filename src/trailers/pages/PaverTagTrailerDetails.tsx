...
import { paverTagTrailer } from "@/trailers/data/trailerData";
import Header from "@/components/Header";
import Footer from "@/components/Footer";

export function PaverTagTrailerDetails() {
  const [activeImage, setActiveImage] = useState(
    paverTagTrailer.gallery[0],
  );
  ...
  const handleDownloadBrochure = () => {
    ...
    pdf.setFontSize(22);
    pdf.text(paverTagTrailer.title, 40, 52);

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(11);
    const descriptionLines = pdf.splitTextToSize(
      paverTagTrailer.description,
      532,
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    paverTagTrailer.specs.forEach((spec) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    paverTagTrailer.details.forEach((detail) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.save(paverTagTrailer.brochureFileName);
  };
      ...
      <div className="pt-24 pb-12 px-6">
        <div className="container mx-auto max-w-6xl">
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="font-display text-4xl lg:text-6xl font-black text-[#bf1e2e] leading-tight text-balance">
              {paverTagTrailer.title}
            </h1>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  <Link
                    to={`/trailers/${paverTagTrailer.slug}/3d`}
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                {paverTagTrailer.specs.slice(11).map((spec, idx) => (
                  <motion.div
                    key={spec.label}
                    initial={{ opacity: 0, x: -10 }}
                    whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <ul className="space-y-2 text-muted-foreground">
              {paverTagTrailer.details.map((item, idx) => (
                <motion.li
                  key={item}
                  initial={{ opacity: 0, x: -10 }}
                  whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {paverTagTrailer.keyAdvantages.map((adv, idx) => (
                <motion.div
                  key={adv}
