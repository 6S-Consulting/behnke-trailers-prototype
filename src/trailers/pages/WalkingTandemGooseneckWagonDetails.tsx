...
export function WalkingTandemGooseneckWagonDetails() {
  const [activeImage, setActiveImage] = useState(
    walkingTandemGooseneckWagon.gallery[0],
  );
  ...
  const handleDownloadBrochure = () => {
    ...
    pdf.setFontSize(22);
    pdf.text(walkingTandemGooseneckWagon.title, 40, 52);

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(11);
    const descriptionLines = pdf.splitTextToSize(
      walkingTandemGooseneckWagon.description,
      532,
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    walkingTandemGooseneckWagon.specs.forEach((spec) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    walkingTandemGooseneckWagon.details.forEach((detail) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.save(walkingTandemGooseneckWagon.brochureFileName);
  };
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  <Link
                    to="/trailers/walking-tandem-gooseneck-wagon/3d"
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                {walkingTandemGooseneckWagon.specs
                  .slice(0, 7)
                  .map((spec, idx) => (
                    <motion.div
                      key={spec.label}
              ...
              <div className="space-y-3">
                {walkingTandemGooseneckWagon.specs.slice(7).map((spec, idx) => (
                  <motion.div
                    key={spec.label}
                    initial={{ opacity: 0, x: -10 }}
                    whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <ul className="space-y-2 text-muted-foreground">
              {walkingTandemGooseneckWagon.details.map((item, idx) => (
                <motion.li
                  key={item}
                  initial={{ opacity: 0, x: -10 }}
                  whileInView={{ opacity: 1, x: 0 }}
