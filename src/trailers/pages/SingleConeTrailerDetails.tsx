...
import { singleConeTrailer } from "@/trailers/data/trailerData";
import Header from "@/components/Header";
import Footer from "@/components/Footer";

export function SingleConeTrailerDetails() {
  const [activeImage, setActiveImage] = useState(
    singleConeTrailer.gallery[0],
  );
  ...
  const handleDownloadBrochure = () => {
    ...
    pdf.setFontSize(22);
    pdf.text(singleConeTrailer.title, 40, 52);

    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(11);
    const descriptionLines = pdf.splitTextToSize(
      singleConeTrailer.description,
      532,
    );
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    singleConeTrailer.specs.forEach((spec) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    singleConeTrailer.details.forEach((detail) => {
      if (y > 740) {
        pdf.addPage();
        y = 50;
      }
    ...
    pdf.save(singleConeTrailer.brochureFileName);
  };
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  <Link
                    to="/trailers/single-cone-trailer/3d"
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                {singleConeTrailer.specs
                  .slice(0, Math.ceil(singleConeTrailer.specs.length / 2))
                  .map((spec, idx) => (
                    <motion.div
                      key={spec.label}
              ...
              <div className="space-y-3">
                {singleConeTrailer.specs.slice(Math.ceil(singleConeTrailer.specs.length / 2)).map((spec, idx) => (
                  <motion.div
                    key={spec.label}
                    initial={{ opacity: 0, x: -10 }}
                    whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <ul className="space-y-2 text-muted-foreground">
              {singleConeTrailer.details.map((item, idx) => (
                <motion.li
                  key={item}
                  initial={{ opacity: 0, x: -10 }}
                  whileInView={{ opacity: 1, x: 0 }}
