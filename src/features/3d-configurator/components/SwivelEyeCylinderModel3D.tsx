...
const HollowTangMount = ({ radius, thickness, color = "#0f172a" }: { radius: number, thickness: number, color?: string }) => {
  ...
  return (
    <group>
      ...
      <mesh position={[blockLength / 2 + 0.22, 0, 0]} castShadow>
        <boxGeometry args={[blockLength, radius * 1, thickness]} />
...
export default function WeldedTangCrossTubeModel3D({ config, dimensions, showDimensions }: Props) {
  ...
  const baseTangWidth = outerR * 1.4;
  ...
  return (
    <group rotation={[0, Math.PI / 2, 0]}>
      ...
      <group position={[-L / 2, 0, 0]} rotation={[0,0,  0]}>
      ...
      {showDimensions && (
        <>
          ...
          <CADMeasurement
            start={[(L / 2 + currentStroke) -1, -rodR, 0]}
            end={[(L / 2 + currentStroke) -1, rodR, 0]}
